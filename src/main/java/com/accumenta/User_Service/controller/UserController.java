...
public class UserController {
    ...
    @GetMapping("user/{userMobileNumber}")
    public ResponseEntity<ResponceWapper<User>> getUserByMobileNumber(@PathVariable long userMobileNumber){
        User dbUser = this.userService.getUserByMobileNumber(userMobileNumber);
        return ResponseEntity.status(HttpStatus.OK).body(new ResponceWapper<>(dbUser));
    }
